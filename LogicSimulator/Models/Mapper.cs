    ...
    public class Mapper {
        ...
        */
        ...
        public void Move(Control item, Point pos)
        {
            ...
            { // То самое место, что не даёт подключить вход ко входу, либо выход к 

                if (marker_circle != null && marker_circle != @ellipse)
                { // На случай моментального перехода курсором с одного кружка на другой
            ...
            // if (mode == 0 && (string?) item.Tag == "Join") {
        ...
        {
            Move(item, pos);
            //Log.Write("PointerReleased: " + item.GetType().Name + " pos: " + pos + " mode " + mode);
            ...
            switch (mode)
            {
                case 5 or 6 or 7:
                    ...
                    if (marker_circle != null)
                    {
                        ...
                        //var a = items.IndexOf(start_dist.parent);
                        //var b = items.IndexOf(end_dist.parent);
                        //if (a == b)
                        //    break;
                        //Log.Write("Стартовый элемент: " + a + " (" + start_dist.GetPos() + ")");
                        //Log.Write("Конечный  элемент: " + b + " (" + end_dist.GetPos()   + ")");
